...
module Yast
  class DslClient < Client
    def main
      ...
      Builtins.y2milestone("DSL module started")
      ...
      Yast.include self, "network/dsl/wizards.rb"
      ...
      if Ops.greater_than(Builtins.size(@args), 0)
        ...
        if Ops.is_path?(WFM.Args(0)) && WFM.Args(0) == path(".capiadsl")
          Builtins.y2milestone("Using CAPI-ADSL mode")
          DSL.capiadsl = nil
          if Ops.greater_than(Builtins.size(@args), 1)
            DSL.capiadsl = Convert.to_map(WFM.Args(1))
          end
          DSL.capiadsl = {} if DSL.capiadsl == nil
        else
          Builtins.y2error("Bad argument for modem: %1", WFM.Args(0))
        end
      end
      ...
      # Finish
      Builtins.y2milestone("DSL module finished")
      Builtins.y2milestone("----------------------------------------")
      deep_copy(@ret) 
