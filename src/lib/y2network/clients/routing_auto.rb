...
module Y2Network
  module Clients
    # This client is reponsible of the autoinstallation routing configuration.
    class RoutingAuto < ::Installation::AutoClient
      ...
      def import(profile = {})
        ...
        config.routing = Y2Network::ConfigReader::AutoyastRouting.new(routing_section).config
        modified!
      end
    ...
    private
      ...
      # Convenience method to obtain the current AutoYaST network
      # configuration
      #
      # @return [Config] current AutoYaST network config
      def config
        @config ||= create_config
      end

      # Convenience method to create a new AutoYaST network configuration
      #
      # @return [Config] AutoYaST network config
      def create_config
        config = Yast::Lan.yast_config
        return config if config

        networking_section = Y2Network::AutoinstProfile::NetworkingSection.new
        config = Y2Network::ConfigReader::Autoyast.new(networking_section).config
