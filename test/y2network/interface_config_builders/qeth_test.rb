...
describe Y2Network::InterfaceConfigBuilders::Qeth do
  subject(:builder) do
  ...
  before do
    ...
    if initialize_channels
      builder.read_channel = "0.0.0800"
      builder.write_channel = "0.0.0801"
      builder.data_channel = "0.0.0802"
    end
  end

  describe "#type" do
    it "returns qeth type" do
  ...
  describe "#configure" do
    it "tries to activate the group device associated with the defined device id" do
      expect(Yast::Execute).to receive(:on_target!)
        .with("/sbin/chzdev", "qeth", builder.device_id, "-e", allowed_exitstatus: 0..255).and_return(0)
      subject.configure
    end
  ...
  describe "#configured_interface" do
    before do
      allow(executor).to receive(:on_target!)
        .with(["/sbin/lszdev", builder.device_id, "-c", "names", "-n"])
        .and_return("eth1")
    end
  ...
  describe "#device_id_from" do
    context "given the read, write or data device id" do
      let(:device_id) { "0.0.0700:0.0.0701:0.0.0702" }
      let(:data_channel) { "0.0.0702" }
      let(:hwinfo) { Y2Network::Hwinfo.new(hwinfo: { "busid" => data_channel }) }
      before do
      ...
      it "obtains the triplet device ids listed by lszdev" do
        expect(subject.device_id_from("0.0.0702")).to eq(device_id)
      end
    end
  end

  describe "#device_id" do
    it "returns the channel, write and data device ids joined by ':'" do
      expect(subject.device_id).to eql("0.0.0800:0.0.0801:0.0.0802")
    end
  end
  ...
  describe "#propose_channels" do
    context "when the read and write channel have not been initialized" do
      ...
      let(:device_id) { "0.0.0800:0.0.0801:0.0802" }
      let(:write_channel) { "0.0.0801" }
      let(:hwinfo) { Y2Network::Hwinfo.new(hwinfo: { "busid" => write_channel }) }

      before do
        allow(builder).to receive(:device_id_from).with(write_channel).and_return(device_id)
        allow(builder).to receive(:hwinfo).and_return(hwinfo)
      end

      it "initializes them from the given busid" do
        expect { builder.propose_channels }.to change { builder.device_id }.from(nil).to(device_id)
      end
    end
  end
