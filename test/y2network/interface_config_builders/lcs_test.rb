...
describe Y2Network::InterfaceConfigBuilders::Lcs do
  ...
  describe "#configure" do
    it "tries to activate the group device associated with the defined device id" do
      expect(Yast::Execute).to receive(:on_target!)
        .with("/sbin/chzdev", "lcs", builder.device_id, "-e",
          "protocol=#{builder.protocol}", "lancmd_timeout=5",
  ...
  describe "#configured_interface" do
    ...
    it "obtains the network interface associated with builder device id" do
      expect(subject.configured_interface).to eq("lcs1")
    end
  end
